...
class QueryDataTable extends DataTableAbstract
{
     ...
     */
    protected function prepareCountQuery():Builder
    {
     ...
     * @return bool
     */
    public function hasFilterColumn($columnName):bool
    {
     ...
     * @return string
     */
    protected function getColumnSearchKeyword($i, $raw = false):string
    {
     ...
     * @return Builder
     */
    protected function getBaseQueryBuilder($instance = null): Builder
    {
        if (! $instance) {
            $instance = $this->query;
        }
     ...
     * @return string
     */
    protected function resolveRelationColumn($column):string
    {
        return $column;
    }
    ...
    {
        $firstChar = substr(str_replace("%", "", $keyword), 0, 1);


        if (is_numeric(str_replace("%", "", $keyword)) && !in_array($firstChar, ['<','>','='])) {
            $firstChar = '=';
            $keyword = $firstChar.$keyword;
        }
        ...
        if (in_array($firstChar, ['<','>','='])) {
            ...
            $this->numberColumnSearch($column, $keyword);
        } elseif ($this->request->isRegex($i)) {
            $column = strstr($column, '(') ? $this->connection->raw($column) : $column;
     ...
     * @param string $keyword
     */
    protected function numberColumnSearch($column, $keyword)
    {

        $cleanKeyword = str_replace("%", "", $keyword);

        $operator = substr($cleanKeyword, 0, 1);
        $number = str_replace($operator, '', $cleanKeyword);
        if ($number=='') {
            $number = 0;
        }
        ...
        if (is_string($column)) {
            ...
            $columnSearch = $column->getValue();
        }



        $sql = $columnSearch . " $operator $number";

        if (stripos($columnSearch, "count")!==false) {
            $this->query->havingRaw($sql, [$number]);
        } else {
            $this->query->whereRaw($sql, [$number]);
        }
    }
    ...
    {
        $sql = $this->config->get('datatables.nulls_last_sql');
