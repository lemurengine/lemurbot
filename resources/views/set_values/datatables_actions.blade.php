<div class='btn-group'  data-test="{$htmlTag}-datatable-actions">
    <a href="{{ route('setValues.show', $id) }}" class='btn btn-info btn-xs show-button' data-test="show-button">
        <i class="glyphicon glyphicon-eye-open"></i>
    </a>
    @if(Auth::user()->id == $user_id || LemurPriv::isAdmin(Auth::user()))
        <a href="{{ route('setValues.edit', $id) }}"  class='btn btn-success btn-xs edit-button' data-test="edit-button">
